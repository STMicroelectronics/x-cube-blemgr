...
void BLE_SetCustomAdvertiseData(uint8_t *manuf_data)
{
  ...
  * For example for the environmental features:
  * 
  *		//Custom advertise data setting for the environmental features
  *		BLE_SetEnvAdvertiseData(manuf_data);
  */
#else /* BLE_MANAGER_SDKV2 */

  manuf_data[BLE_MANAGER_CUSTOM_FIELD1]=0x0D; /* Firmware ID 0x0D for example */
  manuf_data[BLE_MANAGER_CUSTOM_FIELD2]=0xAA; /* First  Option Byte (Fake Value) */
  manuf_data[BLE_MANAGER_CUSTOM_FIELD3]=0xBB; /* Second Option Byte (Fake Value) */
  manuf_data[BLE_MANAGER_CUSTOM_FIELD4]=0xCC; /* Third  Option Byte (Fake Value) */

#warning BLE_MANAGER_SDKV2: BLE_VERSION_FW_ID define not valid
#endif //BLE_VERSION_FW_ID
...
static void WriteRequestOtaFunction(uint8_t *att_data, uint8_t data_length)
{
  BLE_MANAGER_PRINTF("-- OTA SERVICE COMMAND: BOOT REQUESTED\r\n");
