...
static void User_Init(void)
{
  ...
  SENSOR_DT_PRINTF("Debug Notify Trasmission Enabled\r\n\n");
...
static void ComputeRandomQuaternions(void)
{
  ...
  if (counter < 25) {
    quat_axes_send[0].x -= (100  + ((uint64_t)rand()*3*counter)/RAND_MAX);
    quat_axes_send[0].y += (100  + ((uint64_t)rand()*5*counter)/RAND_MAX);
    quat_axes_send[0].z -= (100  + ((uint64_t)rand()*7*counter)/RAND_MAX);
  }
  else {
    quat_axes_send[0].x += (200 + ((uint64_t)rand()*7*counter)/RAND_MAX);
    quat_axes_send[0].y -= (150 + ((uint64_t)rand()*3*counter)/RAND_MAX);
    quat_axes_send[0].z += (10  + ((uint64_t)rand()*5*counter)/RAND_MAX);
  }
  ...
  if (counter == 50) {
    counter = 0;
    quat_axes_send[0].x= -quat_axes_send[0].x;
    quat_axes_send[0].y= -quat_axes_send[0].y;
    quat_axes_send[0].z= -quat_axes_send[0].z;
  }
}
...
void SetRandomEnvironmentalValues(int32_t *PressToSend,uint16_t *HumToSend,int16_t *TempToSend)
{
  ...
  *TempToSend=0;
  ...
  /* P sensor emulation */
  RandomValue = 1000.0 + ((uint64_t)rand()*80)/RAND_MAX;
